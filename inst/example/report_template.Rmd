...
```{r exploratory data analysis, warning=FALSE}
nms <- names(d0)
...
* exposure variable `r sQuote(d0@exposure)` at lag `r exposure.lag`.
...
baseform <- buildFormula(d0, response, exposure, timevar, exposure.lag, tempvar, nyr, 6)
basecall <- substitute(glm(baseform, data = as(d0, "data.frame"), family = poisson), list(baseform = baseform))
base <- eval(basecall)
print(base$call)
```

A key aspect of the model building process involves selecting the optimal degrees of freedom for the smooth function of time to adjust for unmeasured confounding. The approach is based on the methods described in [Dominici et al. (2004)][domi:jasa] and [Peng et al. (2006)][peng:jrssa]. Essentially, we choose the degrees of freedom per year that optimally predicts the variation in the exposure variable (`r sQuote(d0@exposure)`). The fit of the model to the data is measured via AIC and the range of degrees of freedom per year that we search over is between 2 and 20 per year. 
...
finalexpr <- buildFormula(d0, response, exposure, timevar, exposure.lag, tempvar, nyr, dfmin)
finalcall <- substitute(glm(finalexpr, data = as(d0, "data.frame"), family = poisson), list(finalexpr = finalexpr))
final <- eval(finalcall)
print(final$call)
```
...
We conducted a sensitivity analysis with respect to the degrees of freedom for
the smooth function of time. Previous work has shown that the estimate of the regression coefficient for `r sQuote(d0@exposure)` can be sensitive to the number of degrees of freedom used in the time smoother, which is used to model unmeasured time-varying potential confounders. Below we plot the estimate of the coefficient for `r sQuote(d0@exposure)` using degrees of freedom varying from `r min(dfseq)` to `r max(dfseq)` per year of data.
...
ypts <- b
plot(xpts, ypts, ylim = range(ci), xlab = "Models (df/year)", ylab = "Raw coefficient", pch = 19, xaxt = "n", main = "Sensitivity Analysis")
...
```{r convert sensitivity analysis}
bb <- 100 * (exp(b * 10) - 1)
cci <- 100 * (exp(ci * 10) - 1)
imin <- which.min(bb)
imax <- which.max(bb)
minstr <- sprintf("%s (95%% CI: %s, %s)", prettyNum(bb[imin]), 
                  prettyNum(cci[1, imin]), prettyNum(cci[2, imin]))
maxstr <- sprintf("%s (95%% CI: %s, %s)", prettyNum(bb[imax]), 
                  prettyNum(cci[1, imax]), prettyNum(cci[2, imax]))
```

In the sensitivity analysis, the estimates ranged from a `r minstr` percent increase in `r sQuote(d0@response)` to a `r maxstr` percent increase in `r sQuote(d0@response)`.
...
```{r convert influential points analysis}
if(length(infl) > 0L) {
        bb <- 100 * (exp(b * 10) - 1)
        cci <- 100 * (exp(ci * 10) - 1)
        imin <- which.min(bb)
...
`r if(length(infl) > 0L) { sprintf("In this sensitivity analysis, the estimates ranged from a %s percent increase in %s to a %s percent increase in %s", minstr, sQuote(d0@response), maxstr, sQuote(d0@response)) }`.
...
[domi:jasa]: http://www.stanford.edu/~hastie/Papers/dominiciR2.pdf "Improved semiparametric time series models of air pollution and mortality"
[peng:jrssa]: http://onlinelibrary.wiley.com/doi/10.1111/j.1467-985X.2006.00410.x/abstract "Model choice in time series studies of air pollution and mortality"